...
- [编译 opencv wasm](#编译-opencv-wasm)
  - [0. 配置 `emscripten` 环境](#0-配置-emscripten-环境)
  - [1. 编译官方的 `js` 接口](#1-编译官方的-js-接口)
    - [1.1 编译 `opencv`](#11-编译-opencv)
  - [2. 编译静态 `opencv wasm`](#2-编译静态-opencv-wasm)
    - [2.1 构建环境](#21-构建环境)
    - [2.2 编译](#22-编译)
    - [3. 摘抄的编译脚本(防链接丢失)](#3-摘抄的编译脚本防链接丢失)
  - [reference](#reference)
...
emcmake cmake ../ \
  ...
  -DCMAKE_EXE_LINKER_FLAGS="-msimd128 -s USE_PTHREADS=0"
...
## reference
+ [编译 C/C++ 为 WebAssembly](https://developer.mozilla.org/zh-CN/docs/WebAssembly/Guides/C_to_Wasm)
+ [How to build the whole openCV library into a static library?](https://stackoverflow.com/questions/67235824/how-to-build-the-whole-opencv-library-into-a-static-library)
+ [build.sh](https://github.com/webarkit/opencv-em/blob/main/build.sh)
